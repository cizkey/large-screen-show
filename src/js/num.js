...
;(function($, window, document) {
    ...
    var defaul = {
        deVal: 0,       //传入值
        className:'dataNum',   //样式名称
        digit:''    //默认显示几位数字
    };
    function rollNum(obj, options){
        this.obj = obj;
        this.options = $.extend(defaul, options);
        this.init = function(){
             this.initHtmls(obj,defaul);
        }
    }
    rollNum.prototype = {
        initHtmls: function(obj,options){
            var strHtml = '<ul class="' + options.className + ' inrow">';
            var valLen = options.digit ||  (options.deVal + '').length;
            if(obj.find('.'+options.className).length <= 0){
                for(var i = 0; i<  valLen; i++){
            ...
            this.scroNums(options);
        },
        scroNums: function(options){
            var number = options.deVal;
            var $num_item = $('.' + options.className).find('.tt');
            var h = $('.dataBoc').height();
            $num_item.css('transition','all 2s ease-in-out');
